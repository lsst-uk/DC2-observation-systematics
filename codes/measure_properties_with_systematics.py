...
#functions:
def scan_over_tracts(required_pixels, mag_bins, pz_bins, indir, nside=128, dered=False, imaglim = [24,25]):
    """
    ===INPUTS===
    ...
    for ii, tract in enumerate(tract_info[:,0]):
        ...
        if os.path.isfile(catdir) == True:
            ...
            selmag = np.where((imag>imaglim[0])&(imag<=imaglim[1]))[0]
        ...
        if '-' not in key:
            ...
            if 'redshift' in key:
                ...
                for kk in range(len(bins)-1):
                    sel2 = digi==kk+1
                    #need to make sure that the sum is not nan or inf
                    #To compute the mean of z_p-z_t
                    #usedata = fin[1].data[binkey][sel][sel2] - fin[1].data[key][sel][sel2]
                    #To compute mean of z_t
                    usedata = fin[1].data[key][sel][sel2]
                    ind_nan = np.isnan(usedata)
                    ind_inf = np.isinf(usedata)
                    usedata = usedata[(~ind_nan)&(~ind_inf)]
...
def scan_over_tracts_galmap(nside):
    ...
    for ii, tract in enumerate(tract_info[:,0]):
        ...
        catdir='/pscratch/sd/q/qhang/DESC_DC2_obs-dr6/DC2_obj_with_pz-tract-%s.fits'%int(tract)
